...
export function calculateAgencyFeatureScore(input: AgencyFeatureScoreInput) {
  ...
  const score =
    10 +
    cap(input.profileViews, 25) +
    cap(input.packageViews, 20) +
    cap(input.leads * 4, 25) +
    cap(Math.round(conversion * 15), 15) +
    cap(input.ctaEvents, 10) +
    cap(input.publishedPackages * 2, 10) +
    packagePenalty
